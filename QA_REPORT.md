...
- Required files 共 18 项全部存在：入口说明、交付索引、QA 报告、10 份核心 docs、CSV、静态站三件套和验证脚本。
...
- Required files 检查输出：18 项，missing 为 `none`。
- CSV 检查输出：10 行，字段包括 `opportunity`、`target_user`、`job_to_be_done`、`opportunity_score` 等。
- 静态站本地链接检查：18 个本地 `href`/`src` 均存在；8 个站内锚点均存在。
- Markdown 本地链接检查：27 个本地链接均存在。
